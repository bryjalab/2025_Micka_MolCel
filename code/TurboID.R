...
data.export <- data.selected %>% 
  select(PG.ID,
         starts_with("UniProt"),
         matches("^PG(?!.*log2)", perl = TRUE),   # PG columns not ending in log2
         matches("^PG.*log2$"),                   # PG columns ending in log2
         DVL3.specific,
         starts_with("DVL3.wt_ctrl"),
         # starts_with("DVL3.delST_BirA"),       # check which column do we want to export
         starts_with("DVL3.STA_ctrl"),
         # starts_with("DVL3.STE_BirA"),
...
write.csv(data.export, here("outputs", "DVL3-STA_vs_DVL3-wt.csv"), row.names = FALSE)
# save(data.export, file = here("outputs", "DVL3-STA_vs_DVL3-wt.RData"))

 
 # replace BirA with ctrl ?
